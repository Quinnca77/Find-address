...
// Wrapper struct for determining Ordering on f64 primitive type
...
fn find_addr_vec(contents: String) -> Vec<(f64, f64, String)> {
    ...
    // Add every address in the .osm file to addr_vec
    for (line_index, line) in contents_lines.iter().enumerate() {
        if line.contains("addr:city") {
                ...
                // There shouldn't be any other lines on which the street is described, but if there is, then it should be easy to patch
...
fn find_street_name(lat: f64, lon: f64, addr_vec: Vec<(f64, f64, String)>) -> String {
    ...
    // To get the minimal distance, we use the wrapper struct introduced earlier
    let non_nan_vec: Vec<(NonNan, String)> = distance_vec.iter().map(|x| (NonNan::new(x.0).unwrap(), x.1.clone())).collect();
    // We find the minimal value in the wrapper struct (which will be the smallest distance to the closest building) and extract the street_name of this element
